...
export default {
  props: {
    ...
    colorMode: {
      type: String as PropType<colorModeEnumType>,
      required: true,
    },
  },
  data() {},
  computed: {
    colorModeClasses() {
      if (this.colorMode === colorModeEnum.DARK) {
        return 'bg-green-600';
      }
...
<template>
  <div class="chess-square">
    <div
      class="chess-square-inside relative w-full h-full flex place-items-center text"
      :class="colorModeClasses"
    >
      <span
        v-if="showColumnName"
        class="absolute right-1 bottom-1 font-semibold text-black"
        >{{ column }}</span
      >
      <span
        v-if="showRowName"
        class="absolute left-1 top-1 text-bold font-semibold text-black"
        >{{ row }}</span
      >
      <!--
  ...
  -->
      ...
      >
        <text x="10" y="50" font-size="48" fill="white">&#9812;</text>
      </svg>
      -->
      <svg
      ...
      >
        ...
        <defs id="defs20">
          <linearGradient id="linearGradient3757" osb:paint="solid">
            <stop
        ...
        <g
          id="#d9d9d9ff"
          transform="matrix(0.94,0,0,0.94,7.6795174,7.979143)"
          style="fill: #f2f2f2"
        >
          <path
        ...
        <g
          ...
          transform="matrix(0.94,0,0,0.94,7.6795174,7.9791413)"
        >
          <path
